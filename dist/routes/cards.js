...
router.get('/cards', async (req, res) => {
    try {
        ...
        const cardsWithDetails = await Promise.all(cards.map(async (card) => {
            ...
            try {
                const stripeCard = await stripe_1.stripe.issuing.cards.retrieve(card.stripeId);
...
router.post('/cards', async (req, res) => {
    try {
        ...
        const card = await (0, stripeAdapter_1.createVirtualCard)(cardholderId, spendLimitCents);
...
router.patch('/cards/:id/freeze', async (req, res) => {
    try {
        ...
        const updated = freeze
            ? await (0, stripeAdapter_1.freezeCard)(dbCard.stripeId)
            : await (0, stripeAdapter_1.unfreezeCard)(dbCard.stripeId);
...
router.get('/cards/:id/test-credentials', async (req, res) => {
    try {
        ...
        const credentials = await (0, stripeAdapter_1.getCardTestCredentials)(dbCard.stripeId);
...
router.get('/cardholders/:id', async (req, res) => {
    try {
        const cardholder = await (0, stripeAdapter_1.getCardholderDetails)(req.params.id);
        res.json(cardholder);
    }
    catch (err) {
        res.status(400).json({ error: err.message });
...
router.get('/cardholders/:id/balance', async (req, res) => {
    try {
        const balance = await (0, stripeAdapter_1.getFundingSourceBalance)(req.params.id);
        res.json(balance);
    }
    catch (err) {
        res.status(400).json({ error: err.message });
...
router.get('/cards/:id/balance', async (req, res) => {
    try {
        ...
        const balance = await (0, stripeAdapter_1.getCardBalance)(card.stripeId);
        res.json(balance);
    }
    catch (err) {
        res.status(400).json({ error: err.message });
