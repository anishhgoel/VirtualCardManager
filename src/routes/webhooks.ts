...
import { approveAuthorization, declineAuthorization } from '../services/stripeAdapter';
import { PrismaClient, Card, Rule } from '../generated/prisma';
...
async function evaluateRules(
  ...
  for (const rule of rules.filter(r => r.type === 'MERCHANT_CATEGORY')) {
    const merchantData = authorization.merchant_data as any;
  ...
  try {
    ...
    if (event.type === 'issuing_authorization.request') {
      ...
      const dbCard = await prisma.card.findFirst({ 
        where: { stripeId: (authorization.card as unknown) as string }, 
        include: { rules: true } 
      });
      if (!dbCard) {
      ...
      if (result.decision === 'APPROVED') {
        stripeResult = await approveAuthorization(authorization.id);
      } else {
        stripeResult = await declineAuthorization(authorization.id);
      }
